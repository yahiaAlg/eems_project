...
class IndividualSubscribeForm(forms.Form):
    ...
    full_name = forms.CharField(
        label="الاسم واللقب", max_length=150,
        widget=forms.TextInput(attrs={**WIDGET_ATTRS, "placeholder": "مثال: أحمد بلعيد"}),
    )
    ...
    gender = forms.ChoiceField(
        label="الجنس", choices=[("", "اختر")] + list(GENDER_CHOICES), required=False,
    ...
    phone = forms.RegexField(
        ...
        widget=forms.TextInput(attrs={**WIDGET_ATTRS, "placeholder": "0770 12 34 56", "dir": "ltr"}),
    )
    email = forms.EmailField(
        label="البريد الإلكتروني", required=False,
        widget=forms.EmailInput(attrs={**WIDGET_ATTRS, "dir": "ltr", "placeholder": "example@mail.com"}),
    )
    wilaya = forms.CharField(
        label="الولاية", max_length=60, required=False, initial="سطيف",
        widget=forms.TextInput(attrs=WIDGET_ATTRS),
    )
    address = forms.CharField(
        label="العنوان (اختياري)", max_length=255, required=False,
        widget=forms.TextInput(attrs=WIDGET_ATTRS),
    )

    # --- profile / background ---
    education_level = forms.ChoiceField(
        label="المستوى الدراسي", choices=EDUCATION_LEVEL_CHOICES, required=False,
        widget=forms.Select(attrs=SELECT_ATTRS),
    )
    employment_status = forms.ChoiceField(
        label="الوضعية المهنية الحالية", choices=EMPLOYMENT_STATUS_CHOICES, required=False,
        widget=forms.Select(attrs=SELECT_ATTRS),
    )
    preferred_contact_time = forms.ChoiceField(
        label="الوقت المفضل للاتصال", choices=CONTACT_TIME_CHOICES, required=False,
        widget=forms.Select(attrs=SELECT_ATTRS),
    )
    source = forms.ChoiceField(
        label="كيف سمعت عنّا؟", choices=SOURCE_CHOICES, required=False, initial="web",
        widget=forms.Select(attrs=SELECT_ATTRS),
    )

    # --- about this enrollment ---
    motivation = forms.CharField(
        label="ملاحظات / دافع الترشح لهذا التخصص", required=False,
...
class CommentForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "email": forms.EmailInput(attrs={**WIDGET_ATTRS, "placeholder": "بريدك (اختياري)", "dir": "ltr"}),
            "rating": forms.Select(attrs=SELECT_ATTRS),
            "text": forms.Textarea(attrs={**WIDGET_ATTRS, "rows": 3, "placeholder": "شاركنا رأيك حول هذا التخصص..."}),
        }
        labels = {
            "name": "الاسم",
...
class EnquiryForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "name": forms.TextInput(attrs={**WIDGET_ATTRS, "placeholder": "اسمك"}),
            "phone": forms.TextInput(attrs={**WIDGET_ATTRS, "placeholder": "رقم الهاتف (اختياري)", "dir": "ltr"}),
            "email": forms.EmailInput(attrs={**WIDGET_ATTRS, "placeholder": "بريدك الإلكتروني", "dir": "ltr"}),
            "question": forms.Textarea(attrs={**WIDGET_ATTRS, "rows": 3, "placeholder": "اطرح سؤالك حول هذا التخصص، الشروط، أو المواعيد..."}),
        }
        labels = {
            "name": "الاسم",
            "phone": "الهاتف",
