...
IMPORTANT — certificates and the career timeline are only generated for
formateurs that already have a real `title` or `bio` (i.e. an admin has
manually curated that profile). Formateurs imported as-is from the
Trainer CSV export (seed_formateurs) have no title/bio, so they are left
alone here rather than being given a fabricated credential/career history
under their real name. Only the CV-mode "custom upload" demo (a mostly
empty, clearly-watermarked scanned-copy simulation) still runs for one
formateur regardless, since it invents no facts about them.
...
def _seal(draw, cx, cy, r, label="EEMS"):
    ...
    draw.ellipse(
        [cx - r + 9, cy - r + 9, cx + r - 9, cy + r - 9], outline=NAVY, width=2
    )
...
def _certificate_image(title, issuer, recipient, size=(1200, 850)):
    ...
    for corner in [(50, 50), (w - 50, 50), (50, h - 50), (w - 50, h - 50)]:
        draw.ellipse(
            [corner[0] - 5, corner[1] - 5, corner[0] + 5, corner[1] + 5], fill=ACCENT
        )
    ...
    draw.text(
        (w / 2, y + 40), "تشهد بأن", font=_font(16, bold=False), fill=MUTED, anchor="mm"
    )
    draw.text((w / 2, y + 82), recipient, font=_font(34), fill=ACCENT, anchor="mm")
    draw.line(
        [(w / 2 - 180, y + 108), (w / 2 + 180, y + 108)], fill=(226, 232, 240), width=2
    )
    draw.text(
        (w / 2, y + 140),
        "قد حصل/ت على هذا الاعتماد ضمن مسارها المهني والتكويني",
        font=_font(16, bold=False),
        fill=(51, 65, 85),
        anchor="mm",
    )
    draw.text(
        (w / 2, y + 172), issuer, font=_font(15, bold=False), fill=MUTED, anchor="mm"
    )

    # signature line + seal
    draw.line([(90, h - 110), (330, h - 110)], fill=(148, 163, 184), width=2)
    draw.text(
        (210, h - 90), "التوقيع", font=_font(14, bold=False), fill=MUTED, anchor="mm"
    )
    _seal(draw, w - 190, h - 140, 58)

    draw.text(
        (90, h - 60),
        "وثيقة نموذجية تلقائية",
...
def _custom_cv_scan_image(formateur, size=(1000, 1414)):
    ...
    draw.text(
        (margin, 118),
        formateur.title or "مكوّن مهني",
        font=_font(19, bold=False),
        fill=(90, 90, 90),
    )
    draw.line([(margin, 158), (w - margin, 158)], fill=(60, 60, 60), width=2)

    y = 190
    draw.text((margin, y), "نبذة", font=_font(22), fill=(30, 30, 30))
    y += 34
    ...
    draw.text((margin, y), "معلومات", font=_font(22), fill=(30, 30, 30))
    y += 34
    for label, value in [
    ...
    def handle(self, *args, **options):
        ...
        for i, f in enumerate(formateurs):
            ...
            if i == CUSTOM_CV_DEMO_INDEX and not f.cv_file:
                f.cv_mode = "custom"
                f.cv_file.save(
                    f"cv-scan-{f.slug}.jpg", _custom_cv_scan_image(f), save=False
                )
                f.save(update_fields=["cv_mode", "cv_file"])
                cvs += 1

            # --- Certificates (always real JPEG images) ---
            # Only for formateurs an admin has actually curated (title/bio
            # set) — never fabricate a named credential for a formateur
            # that only exists because they're a row in the Trainer CSV.
            is_curated = bool(f.title or f.bio)
            if is_curated and not f.certificates.exists():
                for i2, (title, issuer) in enumerate(CERT_TITLES):
                    cert = FormateurCertificate(
                        formateur=f, title=title, issuer=issuer, order=i2
                    )
                    cert.file.save(
            ...
            # --- Career timeline ---
            # Same rule: a fabricated employment history should never be
            # attached to a real, CSV-imported name with no curated data.
            if is_curated and not f.career_entries.exists():
                ...
                for i3, (role, span) in enumerate(CAREER_TEMPLATES):
                    ...
                    FormateurCareerEntry.objects.create(
                        ...
                        role_title=role,
                        organization=(
                            "مؤسسة التميز للإدارة والسلامة (EEMS)" if span == 0 else ""
                        ),
                        start_year=start,
                        end_year=end,
                        description="",
